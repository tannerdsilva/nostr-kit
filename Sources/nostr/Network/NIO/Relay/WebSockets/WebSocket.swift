...
extension WebSocket {
	...
	internal static func setupChannelForWebScokets(url:URL, splitURL:URL.Split, headers:HTTPHeaders = [:], channel:Channel, wsPromise:EventLoopPromise<Relay>, on eventLoop:EventLoop, configuration:Relay.Client.Configuration) -> EventLoopFuture<Void> {
		...
		let websocketUpgrader = WebSocket.Upgrader(surl:splitURL, url:url, requestKey:base64Key, maxFrameSize:1 << 20, upgradePromise:upgradePromise) { (channel, _) -> EventLoopFuture<Void> in
			...
			let relay = Relay(handler:relayHandler)
			upgradePromise = channel.pipeline.addHandlers([webSocketHandler]).map { wsPromise.succeed(relay) }
